...
interface TestClientNotificationsModalProps {
  isOpen: boolean;
  onClose: () => void;
}

export default function TestClientNotificationsModal({ isOpen, onClose }: TestClientNotificationsModalProps) {
        ...
        <div className="flex items-center justify-between p-6 border-b border-slate-700 bg-slate-800 rounded-t-xl">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-slate-400">Centro de notificaciones para funcionarios</p>
            </div>
            {unreadCount > 0 && (
        ...
        <div className="flex-1 overflow-y-auto p-6 bg-slate-900">
          {loading ? (
            ...
            <div className="text-center py-12">
              ...
              <p className="text-slate-400 mb-6">No se encontraron notificaciones para tu rol: <span className="text-green-400 font-semibold">{userRole}</span></p>
              <Button 
                onClick={loadNotifications} 
              ...
              {notifications.map((notification: any) => (
                <div
                  ...
                  className={`p-5 border rounded-xl hover:bg-slate-800 transition-all duration-200 cursor-pointer group ${
                    !notification.leida 
                      ? 'bg-green-950/30 border-green-600/50 shadow-lg shadow-green-900/20' 
                ...
                >
                  <div className="flex items-start gap-4">
                    <div className="flex-shrink-0 mt-1">
                      <div className={`p-2 rounded-lg ${!notification.leida ? 'bg-green-600/20' : 'bg-slate-700'}`}>
                      ...
                      <div className="flex items-start justify-between mb-3">
                        ...
                        {!notification.leida && (
                          <div className="flex items-center gap-2 ml-4">
                            ...
                            <span className="text-xs text-green-400 font-semibold">NUEVA</span>
                          </div>
                        )}
                      </div>
        ...
        <div className="p-6 border-t border-slate-700 bg-slate-800 rounded-b-xl flex justify-between items-center">
          <div className="flex items-center gap-3">
            ...
            <span className="text-sm text-slate-400">
              ...
              <span className="text-green-400 font-semibold ml-1 capitalize">{userRole}</span>
            </span>
          </div>
          <Button 
            onClick={loadNotifications} 
