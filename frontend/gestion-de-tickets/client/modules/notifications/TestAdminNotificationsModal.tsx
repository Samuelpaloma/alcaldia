...
interface TestAdminNotificationsModalProps {
  isOpen: boolean;
  onClose: () => void;
}

export default function TestAdminNotificationsModal({ isOpen, onClose }: TestAdminNotificationsModalProps) {
      ...
      <div className="bg-slate-900 border border-slate-700 rounded-xl shadow-2xl w-full max-w-5xl h-[90vh] flex flex-col backdrop-blur-sm">
        ...
        <div className="flex items-center justify-between p-6 border-b border-slate-700 bg-slate-800 rounded-t-xl">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="text-xl font-bold text-white">🔧 Panel de Notificaciones - Admin</h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-2">
              ...
              <div className="flex justify-between items-center">
                ...
                <span className="text-blue-400 font-semibold capitalize">{userRole}</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-slate-400">Total notificaciones:</span>
                <span className="text-white font-semibold">{notifications.length}</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-slate-400">No leídas:</span>
            ...
            {debugInfo && (
              <div className="bg-slate-900 rounded-lg p-4 border border-slate-600">
                <h4 className="font-semibold mb-3 text-white">Backend Debug:</h4>
                <div className="space-y-2 text-sm">
                  <div className="flex justify-between">
                    <span className="text-slate-400">Usuario existe:</span>
                    <span className={debugInfo.usuarioExiste ? "text-green-400" : "text-red-400"}>
                      {debugInfo.usuarioExiste ? '✅' : '❌'}
                    </span>
                  </div>
                  {debugInfo.usuarioInfo && (
                    <div className="flex justify-between">
                      <span className="text-slate-400">Tipo usuario:</span>
                      <span className="text-blue-400">{debugInfo.usuarioInfo.tipoUsuario}</span>
                    </div>
                  )}
                  <div className="flex justify-between">
                    <span className="text-slate-400">Total en BD:</span>
                    <span className="text-white">{debugInfo.totalNotificacionesEnBD}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-slate-400">Contiene email:</span>
                    <span className="text-yellow-400">{debugInfo.totalNotificacionesConteniendoEmail}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-slate-400">Específicas:</span>
                    <span className="text-green-400">{debugInfo.notificacionesEspecificas}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-slate-400">Por rol:</span>
                    <span className="text-purple-400">{debugInfo.notificacionesPorRol}</span>
                  </div>
                </div>
                {debugInfo.ejemplosNotificaciones && debugInfo.ejemplosNotificaciones.length > 0 && (
                  <div className="mt-3 pt-3 border-t border-slate-600">
                    <p className="text-slate-400 text-sm mb-2"><strong>Ejemplos de destinatarios:</strong></p>
                    {debugInfo.ejemplosNotificaciones.map((ej: any, index: number) => (
                      <p key={index} className="text-xs text-slate-300 bg-slate-800 p-2 rounded font-mono">
                        {ej.destinatarios}
                      </p>
                    ))}
                  </div>
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {notification.destinatarios && (
                        <div className="mt-3 p-3 bg-slate-800 rounded-lg border border-slate-600">
                          <p className="text-xs text-slate-400 mb-1"><strong>Destinatarios:</strong></p>
                          <p className="text-xs text-slate-300 font-mono bg-slate-900 p-2 rounded">
                            {JSON.stringify(notification.destinatarios)}
                          </p>
                        </div>
                      )}
                    </div>
