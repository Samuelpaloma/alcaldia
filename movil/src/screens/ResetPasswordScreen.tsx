...
export default function ResetPasswordScreen() {
  ...
  const handleResetPassword = async () => {
    ...
    try {
      ...
      if (response.ok) {
        if (data.newPassword) setPasswordError(data.newPassword);
        if (data.confirmPassword) setConfirmPasswordError(data.confirmPassword);
        if (data.token) setCodeError(data.token);
        if (data.message) setGeneralError(data.message);
        // Mostrar mensaje de éxito y navegar al login
        Alert.alert(
          'Contraseña actualizada',
          data.message || 'Tu contraseña ha sido actualizada exitosamente. Ya puedes iniciar sesión.',
          [
            {
              text: 'Continuar',
